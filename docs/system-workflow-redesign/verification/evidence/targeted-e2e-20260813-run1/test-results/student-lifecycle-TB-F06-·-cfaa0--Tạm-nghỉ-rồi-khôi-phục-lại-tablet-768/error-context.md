...
Locator: getByText(/Ghi danh ở lớp .* đã chuyển sang "Tạm nghỉ"/)
Expected: visible
Timeout: 45000ms
...
Call log:
  ...
  - waiting for getByText(/Ghi danh ở lớp .* đã chuyển sang "Tạm nghỉ"/)
...
- main:
  ...
  - navigation:
    ...
    - link "Lịch sử lớp":
      - /url: /students/146fc2b5-bc06-4fce-9838-fb4d37aaf563?tab=history
    - link "Bí tích":
      - /url: /students/146fc2b5-bc06-4fce-9838-fb4d37aaf563?tab=sacraments
    - link "Sức khỏe":
      - /url: /students/146fc2b5-bc06-4fce-9838-fb4d37aaf563?tab=health
  ...
  - form "Đổi người giám hộ của Phêrô Nguyễn Minh Khoa":
    ...
    - combobox "Đổi sang người giám hộ khác":
      ...
      - option "Phụ huynh E2E 127993 · 0912799300"
      - option "Phụ huynh E2E 2 · 84919999992"
      - option "Phụ huynh E2E 825418 · 0982541800"
...
```ts
  ...
  81  |   const href = await link.getAttribute("href");
  82  |   expect(href, `hồ sơ ${fullName} phải có href`).toMatch(/^\/students\/[0-9a-f-]{36}/);
  83  |   await page.goto(href!);
  84  |   if (tab) await page.goto(`${new URL(page.url()).pathname}?tab=${tab}`);
  85  | }
  86  | 
  87  | test.describe("TB-F06 · trạng thái hồ sơ tách khỏi biểu mẫu thông tin", () => {
  88  |   test.beforeEach(async ({ page }) => {
  89  |     await login(page, GROUP_LEADER);
  90  |   });
  91  | 
  92  |   test('biểu mẫu "Cập nhật hồ sơ" KHÔNG còn ô Trạng thái', async ({ page }) => {
  93  |     // 🔴 Đây là điểm trừ C5 = 2 của biên bản audit: trước M03-C ô "Trạng thái"
  94  |     // nằm ngay cạnh ô "Điện thoại", chung một nút "Lưu thay đổi" — lưu trữ một

import { z } from "zod";
import { APP_ROLES, CLASS_ROLES, SECTOR_ROLES } from "@/lib/permissions/roles";
...
export const provisionAccountSchema = z.object({
  ...
  if (CLASS_ROLES.includes(value.role) && !value.staffProfileId) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: "Role lớp cần liên kết hồ sơ nhân sự đã được phân công.", path: ["staffProfileId"] });
  }
